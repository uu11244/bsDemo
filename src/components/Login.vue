      ...
      <el-row type="flex">
        <el-form ref="loginForm" :model="user" :rules="rules">
          ...
          <el-form-item id="password" prop="password">
            <el-input prefix-icon="el-icon-edit" v-model="user.password" show-password placeholder="请输入密码"></el-input>
          </el-form-item>
  ...
  methods: {
    doLogin() {
      ...
      this.$refs.loginForm.validate(async (valid) => {
        // console.log(valid);
        if (!valid) return;
        const { data: res } = await req.require.get('/login', { params: { username: this.user.username, password: this.user.password } })
        console.log(res);
